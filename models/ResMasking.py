"""
Paper: Facial Expression Recognition using Residual Masking Network
Adapted from https://github.com/phamquiluan/ResidualMaskingNetwork/blob/master/models/resmasking.py
"""
...
class ResNet(nn.Module):
    def __init__(
        ...
        # NOTE: strictly set the in_channels = 3 to load the pretrained model -- IGNORE
        self.conv1 = nn.Conv2d(
            IMAGE_CHANNEL, self.inplanes, kernel_size=7, stride=2, padding=3, bias=False
        )
        ...
        # NOTE: strictly set the num_classes = 1000 to load the pretrained model -- IGNORE
...
class ResMasking(ResNet):
    def __init__(self):
        super(ResMasking, self).__init__(
            block=BasicBlock, layers=[3, 4, 6, 3], in_channels=IMAGE_CHANNEL, num_classes=NUM_CLASSES
        )
    ...
    def forward(self, x):  # 224
        b, c, w, h = x.shape #ensure that the dimension is large enough
        if(w < 64 or h < 64):
            pad = nn.ZeroPad2d((64-w)//2)
            x = pad(x)
        x = self.conv1(x)  # 112
        x = self.bn1(x)
        x = self.relu(x)
...
class ResMasking50(ResNet):
    def __init__(self):
        super(ResMasking50, self).__init__(
            block=Bottleneck, layers=[3, 4, 6, 3], in_channels=IMAGE_CHANNEL, num_classes=NUM_CLASSES
        )
